...
const progress = (() => {
  ...
  const progress = () => {
    ...
    info.innerText = `  (${loaded}/${total}) [${parseInt(
      bar.style.width
    ).toFixed(0)}%]`;
...
//       .replace(
//         /\`\`\`(?=\S)(.*?)(?<!\s)\`\`\`/g,
//         '<code class="font-monospace text-dark">$1</code>'
//       );
//   };
...
//                             <p class="text-dark m-0 p-0" style="white-space: pre-line">${convertMarkdownToHTML(
//                               util.escapeHtml(res.data.komentar)
//                             )}</p>
//                         </div>
//                     </div>`;
//         }
...
//                 </div>
//                 <hr class="text-dark my-1">
//                 <p class="text-dark mt-0 mb-1 mx-0 p-0" style="white-space: pre-line">${convertMarkdownToHTML(
//                   util.escapeHtml(data.komentar)
//                 )}</p>
//                 ${innerComment(data)}
//             </div>`;
//     });
